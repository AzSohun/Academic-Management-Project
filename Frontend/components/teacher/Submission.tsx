                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        {(['all', 'pending', 'graded'] as const).map((filter) => (
                            <button
                                key={filter}
                                onClick={() => { setSubmissionFilter(filter); setPage(1); }}
                                className={`px-4 py-1.5 rounded-md text-xs font-medium capitalize border transition cursor-pointer ${submissionFilter === filter ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-slate-950 text-slate-400 border-slate-800 hover:text-slate-200'}`}
                            >
                                {filter}
                            </button>
                        ))}
                        ...
                        <tbody className="divide-y divide-slate-800/60 bg-slate-900/20">
                            {paginatedSubmissions.map((s) => (
                                <tr key={s.id} className="hover:bg-slate-800/30 transition">
                                    ...
                                    <td className="p-4 text-slate-400">{s.assignmentTitle}</td>
                                    ...
                                    <td className="p-4">
                                        {s.filePath ? (
                                            <a href={s.filePath} target="_blank" rel="noreferrer" className="text-emerald-400 hover:underline">View File</a>
                                        ) : (
