...
import { MyClass, Subject, Assignment, Submission, TeacherProfile, extractArrayData } from '@/interfaces/teacher';

import Overview from '@/components/teacher/Overview';
import AssignmentTab from '@/components/teacher/Assignment';
import SubmissionTab from '@/components/teacher/Submission';
import ClassesTab from '@/components/teacher/Classes';
import ProfileTab from '@/components/teacher/Profile';

import EditAssignment from '@/components/modals/teacher/EditAssignment';
import GradeSubmission from '@/components/modals/teacher/GradeSubmission';
import AssignmentModal from '@/components/modals/teacher/Assignment';
import ViewClassInfo from '@/components/modals/teacher/ViewClassInfo';
...
export default function TeacherView() {
    ...
    // Toggle state to force refetch inside AssignmentModal when a grade is submitted
    ...
    const fetchTeacherData = async () => {
        ...
        try {
            const [classesRes, subjectsRes, assignmentsRes, submissionsRes, profileRes] = await Promise.allSettled([
                api.get('/teacher/classes'), api.get('/teacher/subjects'), api.get('/teacher/assignments'),
                api.get('/teacher/submissions'), api.get('/teacher/profile'),
            ]);
            ...
            if (profileRes.status === 'fulfilled') setTeacherProfile(profileRes.value.data?.data || profileRes.value.data);
        } catch {
    ...
    const pendingSubmissionsCount = useMemo(() => {
        return submissions.filter((s) => s.markAssigned === null || !s.status || s.status.toLowerCase() === 'submitted').length;
    }, [submissions]);
        ...
        <div className="h-full w-full bg-slate-950 text-slate-100 flex font-sans antialiased overflow-hidden selection:bg-emerald-500 selection:text-white">
            ...
            <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} transition-all duration-200 bg-slate-900/90 border-r border-slate-800 flex flex-col justify-between shrink-0 h-full z-20`}>
                <div className="flex flex-col h-full">
                    <div className="h-16 px-5 flex items-center justify-between border-b border-slate-800">
                        ...
                        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1.5 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition cursor-pointer">
            ...
            <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
                <header className="h-16 px-8 bg-slate-900/60 border-b border-slate-800 flex items-center justify-between shrink-0">
                    ...
                    <button onClick={fetchTeacherData} disabled={loading} className="px-4 py-2 rounded-md bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-slate-300 text-sm font-medium transition flex items-center gap-2 disabled:opacity-50 cursor-pointer">
                ...
                <main className="flex-1 p-8 space-y-8 w-full overflow-y-auto">
                    {activeTab === 'overview' && <Overview totalClasses={myClasses.length} totalAssignments={assignments.length} pendingSubmissionsCount={pendingSubmissionsCount} gradedSubmissionsCount={submissions.length - pendingSubmissionsCount} handleTabChange={handleTabChange} />}
                    {activeTab === 'assignments' && <AssignmentTab assignments={assignments} myClasses={myClasses} subjectList={subjectList} fetchTeacherData={fetchTeacherData} showStatus={showStatus} onEditAssignment={setEditingAssignment} onViewSubmissions={setViewingAssignmentSubmissions} />}
                    {activeTab === 'submissions' && <SubmissionTab submissions={submissions} onOpenGrading={setGradingSubmission} />}
                    {activeTab === 'classes' && <ClassesTab myClasses={myClasses} onViewStudents={setViewingStudentsClass} />}
                    {activeTab === 'profile' && <ProfileTab teacherProfile={teacherProfile} fetchTeacherData={fetchTeacherData} showStatus={showStatus} />}
                </main>
            </div>
