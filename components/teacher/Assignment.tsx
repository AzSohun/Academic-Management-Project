...
export default function AssignmentTab({ assignments, myClasses, subjectList, fetchTeacherData, showStatus, onEditAssignment, onViewSubmissions }: AssignmentProps) {
    ...
    const handleCreateAssignment = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/teacher/assignments', { title: newTitle, description: newDescription, marks: Number(newMarks), dueDate: newDueDate, isDraft, classDetailsId: selectedClassId, subjectId: selectedSubjectId });
            ...
            fetchTeacherData();
        } catch { showStatus('error', 'Failed to create assignment.'); }
    };
    ...
    const handlePublishAssignment = async (assignment: IAssignment) => {
        ...
        if (result.isConfirmed) {
            try { await api.patch(`/teacher/assignments/${assignment.id}/publish?isDraft=false`); showStatus('success', 'Assignment published!'); fetchTeacherData(); }
            catch { showStatus('error', 'Failed to publish assignment.'); }
        }
    };
    ...
    const handleDeleteAssignment = async (assignment: IAssignment) => {
        ...
        if (result.isConfirmed) {
            try { await api.delete(`/teacher/assignments/${assignment.id}`); showStatus('success', 'Deleted!'); fetchTeacherData(); }
            catch { showStatus('error', 'Could not delete assignment.'); }
        }
    };
        ...
        <div className="space-y-8">
            <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl space-y-5">
                ...
                <form onSubmit={handleCreateAssignment} className="space-y-5">
                    ...
                    <div><label className="block text-xs font-medium text-slate-400 mb-1.5">Description / Instructions</label><textarea placeholder="Write task details..." value={newDescription} onChange={(e) => setNewDescription(e.target.value)} rows={3} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-slate-200 focus:outline-none focus:border-emerald-500" required /></div>
